# Train the ET model based on the given parameters
...
def grid_search_et(train=None, test=None, criterion=None, max_features=None, min_samples_leaf=None, min_samples_split=None, comment=''):
    '''
    Fitting a given training data using ExtraTrees algorithm
    ...
    because this always results in the best ET prediction output
    ...
    print()
    print('='*25)
    print('GRID-SEARCH MODEL: EXTRA TREES')
    ...
    # Instantiating the ET regressor model
    model = ExtraTreesRegressor(n_estimators=ne, warm_start=True, max_depth=None, bootstrap=True, criterion=criterion, max_features=max_features, min_samples_leaf=min_samples_leaf, min_samples_split=min_samples_split)
    ...
    result_csv = '/ssynthesia/ghostcity/git/codename-cyborg/02401.00011_grid-search/grid-search-et-all-features.csv'
...
for i in ['squared_error', 'absolute_error', 'friedman_mse', 'poisson']:
    for j in ['sqrt', 'log2', 1.0, 2]:
        for k in [1, 2, 3]:
            for l in [2, 3, 4]:
                grid_search_et(train=train, test=test, criterion=i, max_features=j, min_samples_leaf=k, min_samples_split=l, comment='Train 1 to 200 test 200 to 250 using 02312.00004_data-filter/bm-dm-st-ue-uz-sorted-clean-normalized.')
